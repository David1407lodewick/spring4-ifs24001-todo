...
public class HomeControllerTest {
    ...
    public String informasiNim(@PathVariable String nim) {
        ...
        switch (prefix) {
            ...
            default: prodi = "Program studi tidak dikenal";
        }

        return String.format(
            "Inforamsi NIM %s:\n>> Program Studi: %s\n>> Angkatan: 20%s\n>> Urutan: %s",
            nim, prodi, angkatan, nomor.replaceFirst("^0+", "")
        );
    }
    ...
    public String perolehanNilai(@PathVariable String strBase64) {
        try {
            ...
            String[] l = d.split("\\r?\\n");
            int i = 0;
            int bPA = Integer.parseInt(l[i++].trim());
            int bT = Integer.parseInt(l[i++].trim());
            int bK = Integer.parseInt(l[i++].trim());
            int bP = Integer.parseInt(l[i++].trim());
            int bUTS = Integer.parseInt(l[i++].trim());
            int bUAS = Integer.parseInt(l[i++].trim());
            ...
            for (; i < l.length; i++) {
                ...
                try {
                    ...
                    switch (j) {
                        case "PA": pPA += np; mPA += nm; break;
                        case "T":  pT += np;  mT += nm; break;
                        case "K":  pK += np;  mK += nm; break;
                        case "P":  pP += np;  mP += nm; break;
                        case "UTS": pUTS += np; mUTS += nm; break;
                        case "UAS": pUAS += np; mUAS += nm; break;
                    }
            ...
            double na = h.c(pp, bPA) + h.c(pt, bT) + h.c(pk, bK)
                      + h.c(pp_proj, bP) + h.c(puts, bUTS) + h.c(puas, bUAS);
    ...
    public String palingTer(@PathVariable String strBase64) {
        try {
            ...
            if (listNilai.isEmpty()) return "Error: Tidak ada data input.";
